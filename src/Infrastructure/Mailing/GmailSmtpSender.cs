...
public class GmailSmtpSender : IEmailSender
{
    ...
    public async Task<SucceededOrNotResponse> SendAsync<TModel>(TModel model) where TModel : NotificationModel
    {
        try
        {
            ...
            using var smtpClient = new SmtpClient(_gmailSettings.SmtpHost, _gmailSettings.SmtpPort)
            {
                EnableSsl = _gmailSettings.EnableSsl,
                Credentials = new NetworkCredential(_gmailSettings.EmailAddress, _gmailSettings.AppPassword)
            };
        ...
        {
            _logger.LogError(ex, "Error occured while sending email with Gmail SMTP.");
            return new SucceededOrNotResponse(false, [
                new Error("GmailSmtpError", ex.Message)
            ]);
        }
    }
    ...
    private static (string subject, string htmlBody) BuildForgotPasswordContent(ForgotPasswordNotificationModel model)
    {
        ...
        var subject = isFr ? "Reinitialisation du mot de passe" : "Reset your password";
        var body = isFr
            ? $"<p>Bonjour,</p><p>Utilisez ce lien pour reinitialiser votre mot de passe:</p><p><a href=\"{model.Link}\">{model.Link}</a></p>"
    ...
    {
        var isFr = model.Locale == "fr";
        var subject = isFr ? "Accès à votre espace personnel" : "Access your personal space";
        var body = isFr
            ? $"<p>Bonjour,</p><p>Utilisez ce lien pour accéder à votre espace personnel :</p><p><a href=\"{model.Link}\">{model.Link}</a></p>"
            : $"<p>Hello,</p><p>Use this link to access your personal space:</p><p><a href=\"{model.Link}\">{model.Link}</a></p>";
        return (subject, body);
    }
}
