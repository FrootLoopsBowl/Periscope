  ...
  <div class="navbar">
    <template v-if="$windowWidth < 1225">
      ...
      >
        ...
        <IconChevron
          :class="{ 'icon--rotate-180': !isExpanded }"
          class="icon icon--white"
        />
      </button>
    </template>
    <img class="navbar__logo" :src="Logo" alt="Periscope logo" />
    ...
    <Transition name="expand">
      ...
      >
        <router-link to="/dashboard" class="navbar__brand">
          PERISCOPE
        </router-link>

        <router-link to="/dashboard" class="navbar__item">
          Dashboard
        </router-link>
